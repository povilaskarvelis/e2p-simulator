...
// DCA threshold range variables
let dcaThresholdMin = 0.05; // 5%
let dcaThresholdMax = 0.25; // 25%
...
function cleanupContinuous() {
    ...
    if (document.getElementById(SELECTORS.dcaPlot)) {
    ...
    trueLabeledData = [];
    observedLabeledData = [];
    dcaThresholdMin = 0.05;
    dcaThresholdMax = 0.25;
}
...
function plotROC() {
    ...
    // Plot DCA (only if container exists)
    if (document.getElementById(SELECTORS.dcaPlot)) {
        plotDCA();
    }
}
...
function plotDCA() {
    if (!currentLabeledData || currentLabeledData.length === 0) {
        console.warn("plotDCA called with no currentLabeledData.");
        return;
    }
    
    try {
        // Get base rate from the data
...
function updateShadedArea() {
    try {
        ...
        Plotly.relayout(SELECTORS.dcaPlot, {
            ...
            'annotations[0].text': `A-NBC:<br>${anbc.toFixed(2)}`
        });
